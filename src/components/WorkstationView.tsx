...
const WorkstationView: React.FC<WorkstationViewProps> = ({
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const tasksWithProjectInfo = await Promise.all(activeTasks.map(async task => {
        try {
          const {
            data: phaseData,
            error: phaseError
          } = await supabase.from('phases').select('project_id, name').eq('id', task.phase_id).single();
          if (phaseError) throw phaseError;
          const {
            data: projectData,
            error: projectError
          } = await supabase.from('projects').select('name').eq('id', phaseData.project_id).single();
          ...
          if (task.status === 'IN_PROGRESS' && task.assignee_id) {
            const {
              data: employeeData,
              error: employeeError
            } = await supabase.from('employees').select('name').eq('id', task.assignee_id).single();
          ...
          if (task.status === 'IN_PROGRESS') {
            const {
              ...
              error: regError
            } = await supabase.from('time_registrations').select('id').eq('task_id', task.id).eq('is_active', true);
      ...
      if (!workstationDbId && actualWorkstationName) {
        ...
        } = await supabase.from('workstations').select('id').eq('name', actualWorkstationName).single();
        ...
        if (rushOrders.length > 0) {
          for (const rushOrder of rushOrders) {
            if (rushOrder.tasks && rushOrder.tasks.length > 0) {
              const tasksWithRushOrderInfo = await Promise.all(rushOrder.tasks.map(async (taskLink: any) => {
                try {
                  const {
                    data: task,
                    error: taskError
                  } = await supabase.from('tasks').select('*').eq('id', taskLink.standard_task_id).single();
                  if (taskError) throw taskError;
                  const {
                    data: rushOrderInfo,
                    error: rushOrderError
                  } = await supabase.from('rush_orders').select('title').eq('id', taskLink.rush_order_id).neq('status', 'completed').single();
                  if (rushOrderError) {
                    return null;
                  }
  ...
  const queryErrorMessage = queryError ? t('failed_to_load_projects', {
    message: ''
  }) : null;
  ...
  const checkAndUpdateLimitPhases = async (completedTask: ExtendedTask) => {
    try {
      ...
      const {
        data: phaseData,
        error: phaseError
      } = await supabase.from('phases').select('project_id').eq('id', completedTask.phase_id).single();
  ...
  const handleStartWorkstationTask = async (workstationTask: any) => {
    ...
    try {
      ...
      toast({
        title: t('workstation_task_started'),
        description: t('workstation_task_started_desc', {
          taskName: workstationTask.task_name
        })
      });
    } catch (error) {
  ...
  return <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold">{t('workstation_view_title', {
          name: actualWorkstationName
        })}</h1>
      ...
      <div className="space-y-6">
        <Card>
          ...
          <CardContent>
            {inProgressTasks.length > 0 ? <div className="space-y-3">
              ...
              return <div key={task.id} className={`border rounded-lg p-4 relative transition-opacity ${isCompleting ? 'opacity-50' : ''}`} style={{
                borderLeftWidth: '4px',
                ...
                borderLeftStyle: 'solid'
                      ...
                      <div className="flex justify-between items-start">
                        <div className="flex-1">
                          ...
                          {task.assignee_name && <p className="text-sm text-blue-600">{t('assigned_to_label', {
                        name: task.assignee_name
                      })}</p>}
                          {task.due_date && <div className="flex items-center gap-2 mt-1">
                              <p className="text-sm text-gray-500">
                                {t('due_date_label', {
                          date: format(new Date(task.due_date), 'MMM dd, yyyy')
                        })}
                              </p>
                              {urgency && <Badge variant={urgency.variant} className="text-xs">
                                  {urgency.class === 'overdue' && <AlertTriangle className="h-3 w-3 mr-1" />}
                                  {urgency.label}
                                </Badge>}
                            </div>}
                          {task.timeRemaining && <p className={`text-sm font-mono ${task.isOvertime ? 'text-red-600' : 'text-green-600'}`}>
                        ...
                        <div className="flex items-center gap-2">
                          {task.active_workers && task.active_workers > 0 && <div className="flex items-center gap-1 text-sm text-blue-600">
                              <Users className="h-4 w-4" />
                              <span>{task.active_workers}</span>
                            </div>}
                          <Button variant="outline" size="sm" onClick={() => handleShowProjectParts(task)} title={t('view_parts')}>
                            <Package className="h-4 w-4" />
                            {t('view_parts')}
                          </Button>
                          {task.project_id && <Button variant="outline" size="sm" onClick={() => handleGoToFiles(task)} title={t('view_files')}>
                              <FileText className="h-4 w-4" />
                              {t('view_files')}
                            </Button>}
                          {task.project_id && <Button variant="outline" size="sm" onClick={() => handleShowBarcode(task)} title={t('show_barcode')}>
                              <Barcode className="h-4 w-4" />
                              {t('show_barcode')}
                            </Button>}
                          {task.project_id && <Button variant="outline" size="sm" onClick={() => navigate(createLocalizedPath(`/projects/${task.project_id}`))} title={t('go_to_project')}>
                              <ExternalLink className="h-4 w-4" />
                              {t('go_to_project')}
                            </Button>}
                          <button onClick={() => handleJoinTask(task.id)} className="px-3 py-1 bg-blue-500 text-white rounded text-sm hover:bg-blue-600" disabled={isCompleting}>
                            {t('join_task')}
                          </button>
                          <button onClick={() => handleTaskUpdate(task.id, 'COMPLETED')} className="px-3 py-1 bg-green-500 text-white rounded text-sm hover:bg-green-600 flex items-center gap-1" disabled={isCompleting}>
              ...
              </div> : <p className="text-gray-500 text-center py-4">{t('no_tasks_in_progress')}</p>}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            {todoTasks.length > 0 ? <div className="space-y-3">
                {todoTasks.map(task => {
              const urgency = task.due_date ? getUrgencyClass(task.due_date) : null;
              const taskColor = getTaskColor(task);
              return <div key={task.id} className="border rounded-lg p-4 relative" style={{
                borderLeftWidth: '4px',
                borderLeftColor: taskColor || '#e5e7eb',
                borderLeftStyle: 'solid'
              }}>
                      <div className="flex justify-between items-start">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => handleTaskUpdate(task.id, 'IN_PROGRESS')} className="px-3 py-1 bg-blue-500 text-white rounded text-sm hover:bg-blue-600">
                            {t('start')}
                          </button>
                        </div>
                      </div>
              ...
              </div> : <p className="text-gray-500 text-center py-4">{t('no_todo_tasks')}</p>}
          </CardContent>
        </Card>
      </div>

      {/* Project Files Popup */}
      {showProjectFiles && <ProjectFilesPopup isOpen={showProjectFiles} onClose={() => setShowProjectFiles(false)} projectId={selectedProjectId} projectName={selectedProjectName} />}

      {/* Task Parts List Viewer */}
      {showPartsListViewer && selectedTaskForParts && <PartsListViewer isOpen={showPartsListViewer} onClose={() => setShowPartsListViewer(false)} taskId={selectedTaskForParts.id} taskTitle={selectedTaskForParts.title} />}

      {/* Project Parts List Dialog */}
      {showPartsListDialog && <PartsListDialog isOpen={showPartsListDialog} onClose={() => setShowPartsListDialog(false)} projectId={selectedProjectId} />}

      {/* Project Barcode Dialog */}
      {showBarcodeDialog && <ProjectBarcodeDialog isOpen={showBarcodeDialog} onClose={() => setShowBarcodeDialog(false)} projectId={selectedProjectId} projectName={selectedProjectName} />}
    </div>;
};
