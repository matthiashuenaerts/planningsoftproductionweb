...
export const ProjectChat: React.FC<ProjectChatProps> = ({
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      setIsSending(true);
      await projectChatService.sendMessage(projectId, newMessage.trim(), selectedFile || undefined);
      setNewMessage('');
  ...
  }, [isOpen, projectId, onUnreadCountChange]);
  ...
  const renderMessage = (message: ProjectMessage) => {
    ...
    return (
      <div
        key={message.id}
        className={`flex gap-3 ${isOwn ? 'flex-row-reverse' : ''} mb-4`}
      >
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px] sm:max-h-[700px] flex flex-col">
        <DialogHeader>
          <DialogTitle>Project Chat - {projectName}</DialogTitle>
        </DialogHeader>
