...
const TaskTimer = () => {
  ...
  const handleExtraTimeConfirm = async (totalMinutes: number) => {
    ...
    const taskId = activeRegistration?.task_id;
    const workstationTaskId = activeRegistration?.workstation_task_id;

    try {
      // Stop the time registration first (preserves original start_time for accurate duration)
      await timeRegistrationService.stopTask(pendingStopData.registrationId);

      // AFTER stopping, set the new duration for the next session
      // This overwrites any remaining duration calculation from stopTask
      if (taskId) {
        await supabase
          .from('tasks')
