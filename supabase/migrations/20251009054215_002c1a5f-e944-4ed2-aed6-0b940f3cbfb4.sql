...
-- Add language column to email_schedule_configs
ALTER TABLE email_schedule_configs 
ADD COLUMN IF NOT EXISTS language text NOT NULL DEFAULT 'nl';

-- Create function to invoke the send-project-forecast edge function
...
BEGIN
  ...
  v_current_day := lower(to_char(CURRENT_TIMESTAMP, 'Day'));
