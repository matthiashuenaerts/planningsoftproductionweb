...
async function syncTenant(supabase: any, tenantId: string) {
  ...
  try {
    ...
    // Snapshot existing hidden ordernummers BEFORE making any changes,
    // so we can preserve the team's hide preferences across re-sync.
    // Trim keys for robust matching (FileMaker sometimes returns padded strings).
    ...
    const hiddenOrderNums = new Set(
      (hiddenRows || []).map((r: any) => String(r.ordernummer).trim()),
    );
    console.log(
      `Tenant ${tenantId}: preserving hidden flag for ${hiddenOrderNums.size} ordernummers`,
    );

    // Re-apply hidden flag to incoming rows (compare on trimmed values)
    let preservedHiddenCount = 0;
    for (const r of rows) {
      if (hiddenOrderNums.has(String(r.ordernummer).trim())) {
        (r as any).hidden = true;
        preservedHiddenCount++;
      } else {
        (r as any).hidden = false;
      }
    }
    console.log(
      `Tenant ${tenantId}: re-applied hidden=true to ${preservedHiddenCount} of ${rows.length} incoming rows`,
    );

    // Replace this tenant's buffer atomically (delete + insert).
    // Hidden flag is preserved via the snapshot above.
